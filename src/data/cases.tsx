import Link from 'next/link';
import Image from 'next/image';

import { CasesPost } from '@/components/cases/interfaces';

// const casesData: CasesPost[] = [
//   {
//     id: 'polyinvest',
//     title: 'Полиинвест',
//     tags: ['лендинг', 'ux/ui дизайн', 'wordpress', 'бот'],
//     description:
//       'Разработка лендинга для профессионального портфельного управляющего с целью рассказа о частном автоследовании, его преимуществах и привлечении новых клиентов.',
//     preview: '/assets/images/cases/polyinvest/preview.jpg',
//     link: {
//       url: 'https://antonpolyakov.ru/',
//       text: 'antonpolyakov.ru',
//     },
//     content: (
//       <>
//         <h2>Заголовок второго уровня</h2>
//         <p>
//           t is a long established fact that a reader will be distracted by the readable content of a page when looking
//           at its layout. The point of using Lorem Ipsum is that it has a more-or-less normal distribution of letters, as
//           opposed to using Content here, content here, making it look like readable English. Many desktop publishing
//           packages and web page editors now use Lorem Ipsum as their default model text, and a search for lorem ipsum
//           will uncover many web sites still in their infancy. Various versions have evolved over the years, sometimes by
//           accident, sometimes on purpose (injected humour and the like).
//         </p>
//         <Image src={'/assets/images/placeholder.png'} width={300} height={150} alt={''} />
//         <h2>Заголовок второго уровня</h2>
//         <Image src={'/assets/images/placeholder.png'} width={300} height={150} alt={''} />
//         <h2>Заголовок второго уровня</h2>
//         <p>
//           t is a long established fact that a reader will be distracted by the readable content of a page when looking
//           at its layout. The point of using Lorem Ipsum is that it has a more-or-less normal distribution of letters, as
//           opposed to using Content here, content here, making it look like readable English. Many desktop publishing
//           packages and web page editors now use Lorem Ipsum as their default model text, and a search for lorem ipsum
//           will uncover many web sites still in their infancy. Various versions have evolved over the years, sometimes by
//           accident, sometimes on purpose (injected humour and the like).
//         </p>
//         <Image src={'/assets/images/placeholder.png'} width={300} height={150} alt={''} />
//         <p>
//           t is a long established fact that a reader will be distracted by the readable content of a page when looking
//           at its layout. The point of using Lorem Ipsum is that it has a more-or-less normal distribution of letters, as
//           opposed to using Content here, content here, making it look like readable English. Many desktop publishing
//           packages and web page editors now use Lorem Ipsum as their default model text, and a search for lorem ipsum
//           will uncover many web sites still in their infancy. Various versions have evolved over the years, sometimes by
//           accident, sometimes on purpose (injected humour and the like).
//         </p>
//         <p>
//           t is a long established fact that a reader will be distracted by the readable content of a page when looking
//           at its layout. The point of using Lorem Ipsum is that it has a more-or-less normal distribution of letters, as
//           opposed to using Content here, content here, making it look like readable English. Many desktop publishing
//           packages and web page editors now use Lorem Ipsum as their default model text, and a search for lorem ipsum
//           will uncover many web sites still in their infancy. Various versions have evolved over the years, sometimes by
//           accident, sometimes on purpose (injected humour and the like).
//         </p>
//         <h2>Заголовок второго уровня</h2>
//         <p>
//           t is a long established fact that a reader will be distracted by the readable content of a page when looking
//           at its layout. The point of using Lorem Ipsum is that it has a more-or-less normal distribution of letters, as
//           opposed to using Content here, content here, making it look like readable English. Many desktop publishing
//           packages and web page editors now use Lorem Ipsum as their default model text, and a search for lorem ipsum
//           will uncover many web sites still in their infancy. Various versions have evolved over the years, sometimes by
//           accident, sometimes on purpose (injected humour and the like).
//         </p>
//         <ul>
//           <li>item item</li>
//           <li>item item</li>
//           <li>item item</li>
//           <li>item item</li>
//         </ul>
//         <ol>
//           <li>item item</li>
//           <li>item item</li>
//           <li>item item</li>
//           <li>item item</li>
//         </ol>
//         <h2>Заголовок второго уровня</h2>
//         <ul>
//           <li>item item</li>
//           <li>item item</li>
//           <li>item item</li>
//           <li>item item</li>
//         </ul>
//       </>
//     ),
//   },
//   {
//     id: 'polyinvest2',
//     title: 'Полиинвест2',
//     tags: ['лендинг', 'ux/ui дизайн', 'wordpress', 'бот'],
//     description:
//       'Разработка лендинга для профессионального портфельного управляющего с целью рассказа о частном автоследовании, его преимуществах и привлечении новых клиентов.',
//     preview: '/assets/images/cases/polyinvest/preview.jpg',
//     link: {
//       url: 'https://antonpolyakov.ru/',
//       text: 'antonpolyakov.ru',
//     },
//     content: (
//       <>
//         <h2>Заголовок второго уровня</h2>
//         <p>
//           t is a long established fact that a reader will be distracted by the readable content of a page when looking
//           at its layout. The point of using Lorem Ipsum is that it has a more-or-less normal distribution of letters, as
//           opposed to using Content here, content here, making it look like readable English. Many desktop publishing
//           packages and web page{' '}
//           <Link className={'accent-link'} href={'#'} target={'_blank'}>
//             editors
//           </Link>{' '}
//           now use Lorem Ipsum as their default model text, and a search for lorem ipsum will uncover many web sites
//           still in their infancy. Various versions have evolved over the years, sometimes by accident, sometimes on
//           purpose (injected humour and the like).
//         </p>
//         <Image src={'/assets/images/placeholder.png'} width={300} height={150} alt={''} />
//         <h2>Заголовок второго уровня</h2>
//         <Image src={'/assets/images/placeholder.png'} width={300} height={150} alt={''} />
//         <h2>Заголовок второго уровня</h2>
//         <p>
//           t is a long established fact that a reader will be distracted by the readable content of a page when looking
//           at its layout. The point of using Lorem Ipsum is that it has a more-or-less normal distribution of letters, as
//           opposed to using Content here, content here, making it look like readable English. Many desktop publishing
//           packages and web page editors now use Lorem Ipsum as their default model text, and a search for lorem ipsum
//           will uncover many web sites still in their infancy. Various versions have evolved over the years, sometimes by
//           accident, sometimes on purpose (injected humour and the like).
//         </p>
//         <Image src={'/assets/images/placeholder.png'} width={300} height={150} alt={''} />
//         <p>
//           t is a long established fact that a reader will be distracted by the readable content of a page when looking
//           at its layout. The point of using Lorem Ipsum is that it has a more-or-less normal distribution of letters, as
//           opposed to using Content here, content here, making it look like readable English. Many desktop publishing
//           packages and web page editors now use Lorem Ipsum as their default model text, and a search for lorem ipsum
//           will uncover many web sites still in their infancy. Various versions have evolved over the years, sometimes by
//           accident, sometimes on purpose (injected humour and the like).
//         </p>
//         <p>
//           t is a long established fact that a reader will be distracted by the readable content of a page when looking
//           at its layout. The point of using Lorem Ipsum is that it has a more-or-less normal distribution of letters, as
//           opposed to using Content here, content here, making it look like readable English. Many desktop publishing
//           packages and web page editors now use Lorem Ipsum as their default model text, and a search for lorem ipsum
//           will uncover many web sites still in their infancy. Various versions have evolved over the years, sometimes by
//           accident, sometimes on purpose (injected humour and the like).
//         </p>
//         <h2>Заголовок второго уровня</h2>
//         <p>
//           t is a long established fact that a reader will be distracted by the readable content of a page when looking
//           at its layout. The point of using Lorem Ipsum is that it has a more-or-less normal distribution of letters, as
//           opposed to using Content here, content here, making it look like readable English. Many desktop publishing
//           packages and web page editors now use Lorem Ipsum as their default model text, and a search for lorem ipsum
//           will uncover many web sites still in their infancy. Various versions have evolved over the years, sometimes by
//           accident, sometimes on purpose (injected humour and the like).
//         </p>
//         <ul>
//           <li>item item</li>
//           <li>item item</li>
//           <li>item item</li>
//           <li>item item</li>
//         </ul>
//         <ol>
//           <li>item item</li>
//           <li>item item</li>
//           <li>item item</li>
//           <li>item item</li>
//         </ol>
//         <h2>Заголовок второго уровня</h2>
//         <ul>
//           <li>item item</li>
//           <li>item item</li>
//           <li>item item</li>
//           <li>item item</li>
//         </ul>
//       </>
//     ),
//   },
//   {
//     id: 'polyinvest3',
//     title: 'Полиинвест3',
//     tags: ['лендинг', 'ux/ui дизайн', 'wordpress', 'бот'],
//     description:
//       'Разработка лендинга для профессионального портфельного управляющего с целью рассказа о частном автоследовании, его преимуществах и привлечении новых клиентов.',
//     preview: '/assets/images/cases/polyinvest/preview.jpg',
//     link: {
//       url: 'https://antonpolyakov.ru/',
//       text: 'antonpolyakov.ru',
//     },
//     content: (
//       <>
//         <h2>Заголовок второго уровня</h2>
//         <p>
//           t is a long established fact that a reader will be distracted by the readable content of a page when looking
//           at its layout. The point of using Lorem Ipsum is that it has a more-or-less normal distribution of letters, as
//           opposed to using Content here, content here, making it look like readable English. Many desktop publishing
//           packages and web page editors now use Lorem Ipsum as their default model text, and a search for lorem ipsum
//           will uncover many web sites still in their infancy. Various versions have evolved over the years, sometimes by
//           accident, sometimes on purpose (injected humour and the like).
//         </p>
//         <Image src={'/assets/images/placeholder.png'} width={300} height={150} alt={''} />
//         <h2>Заголовок второго уровня</h2>
//         <Image src={'/assets/images/placeholder.png'} width={300} height={150} alt={''} />
//         <h2>Заголовок второго уровня</h2>
//         <p>
//           t is a long established fact that a reader will be distracted by the readable content of a page when looking
//           at its layout. The point of using Lorem Ipsum is that it has a more-or-less normal distribution of letters, as
//           opposed to using Content here, content here, making it look like readable English. Many desktop publishing
//           packages and web page editors now use Lorem Ipsum as their default model text, and a search for lorem ipsum
//           will uncover many web sites still in their infancy. Various versions have evolved over the years, sometimes by
//           accident, sometimes on purpose (injected humour and the like).
//         </p>
//         <Image src={'/assets/images/placeholder.png'} width={300} height={150} alt={''} />
//         <p>
//           t is a long established fact that a reader will be distracted by the readable content of a page when looking
//           at its layout. The point of using Lorem Ipsum is that it has a more-or-less normal distribution of letters, as
//           opposed to using Content here, content here, making it look like readable English. Many desktop publishing
//           packages and web page editors now use Lorem Ipsum as their default model text, and a search for lorem ipsum
//           will uncover many web sites still in their infancy. Various versions have evolved over the years, sometimes by
//           accident, sometimes on purpose (injected humour and the like).
//         </p>
//         <p>
//           t is a long established fact that a reader will be distracted by the readable content of a page when looking
//           at its layout. The point of using Lorem Ipsum is that it has a more-or-less normal distribution of letters, as
//           opposed to using Content here, content here, making it look like readable English. Many desktop publishing
//           packages and web page editors now use Lorem Ipsum as their default model text, and a search for lorem ipsum
//           will uncover many web sites still in their infancy. Various versions have evolved over the years, sometimes by
//           accident, sometimes on purpose (injected humour and the like).
//         </p>
//         <h2>Заголовок второго уровня</h2>
//         <p>
//           t is a long established fact that a reader will be distracted by the readable content of a page when looking
//           at its layout. The point of using Lorem Ipsum is that it has a more-or-less normal distribution of letters, as
//           opposed to using Content here, content here, making it look like readable English. Many desktop publishing
//           packages and web page editors now use Lorem Ipsum as their default model text, and a search for lorem ipsum
//           will uncover many web sites still in their infancy. Various versions have evolved over the years, sometimes by
//           accident, sometimes on purpose (injected humour and the like).
//         </p>
//         <ul>
//           <li>item item</li>
//           <li>item item</li>
//           <li>item item</li>
//           <li>item item</li>
//         </ul>
//         <ol>
//           <li>item item</li>
//           <li>item item</li>
//           <li>item item</li>
//           <li>item item</li>
//         </ol>
//         <h2>Заголовок второго уровня</h2>
//         <ul>
//           <li>item item</li>
//           <li>item item</li>
//           <li>item item</li>
//           <li>item item</li>
//         </ul>
//       </>
//     ),
//   },
//   {
//     id: 'polyinvest4',
//     title: 'Полиинвест4',
//     tags: ['лендинг', 'ux/ui дизайн', 'wordpress', 'бот'],
//     description:
//       'Разработка лендинга для профессионального портфельного управляющего с целью рассказа о частном автоследовании, его преимуществах и привлечении новых клиентов.',
//     preview: '/assets/images/cases/polyinvest/preview.jpg',
//     link: {
//       url: 'https://antonpolyakov.ru/',
//       text: 'antonpolyakov.ru',
//     },
//     content: (
//       <>
//         <h2>Заголовок второго уровня</h2>
//         <p>
//           t is a long established fact that a reader will be distracted by the readable content of a page when looking
//           at its layout. The point of using Lorem Ipsum is that it has a more-or-less normal distribution of letters, as
//           opposed to using Content here, content here, making it look like readable English. Many desktop publishing
//           packages and web page editors now use Lorem Ipsum as their default model text, and a search for lorem ipsum
//           will uncover many web sites still in their infancy. Various versions have evolved over the years, sometimes by
//           accident, sometimes on purpose (injected humour and the like).
//         </p>
//         <Image src={'/assets/images/placeholder.png'} width={300} height={150} alt={''} />
//         <h2>Заголовок второго уровня</h2>
//         <Image src={'/assets/images/placeholder.png'} width={300} height={150} alt={''} />
//         <h2>Заголовок второго уровня</h2>
//         <p>
//           t is a long established fact that a reader will be distracted by the readable content of a page when looking
//           at its layout. The point of using Lorem Ipsum is that it has a more-or-less normal distribution of letters, as
//           opposed to using Content here, content here, making it look like readable English. Many desktop publishing
//           packages and web page editors now use Lorem Ipsum as their default model text, and a search for lorem ipsum
//           will uncover many web sites still in their infancy. Various versions have evolved over the years, sometimes by
//           accident, sometimes on purpose (injected humour and the like).
//         </p>
//         <Image src={'/assets/images/placeholder.png'} width={300} height={150} alt={''} />
//         <p>
//           t is a long established fact that a reader will be distracted by the readable content of a page when looking
//           at its layout. The point of using Lorem Ipsum is that it has a more-or-less normal distribution of letters, as
//           opposed to using Content here, content here, making it look like readable English. Many desktop publishing
//           packages and web page editors now use Lorem Ipsum as their default model text, and a search for lorem ipsum
//           will uncover many web sites still in their infancy. Various versions have evolved over the years, sometimes by
//           accident, sometimes on purpose (injected humour and the like).
//         </p>
//         <p>
//           t is a long established fact that a reader will be distracted by the readable content of a page when looking
//           at its layout. The point of using Lorem Ipsum is that it has a more-or-less normal distribution of letters, as
//           opposed to using Content here, content here, making it look like readable English. Many desktop publishing
//           packages and web page editors now use Lorem Ipsum as their default model text, and a search for lorem ipsum
//           will uncover many web sites still in their infancy. Various versions have evolved over the years, sometimes by
//           accident, sometimes on purpose (injected humour and the like).
//         </p>
//         <h2>Заголовок второго уровня</h2>
//         <p>
//           t is a long established fact that a reader will be distracted by the readable content of a page when looking
//           at its layout. The point of using Lorem Ipsum is that it has a more-or-less normal distribution of letters, as
//           opposed to using Content here, content here, making it look like readable English. Many desktop publishing
//           packages and web page editors now use Lorem Ipsum as their default model text, and a search for lorem ipsum
//           will uncover many web sites still in their infancy. Various versions have evolved over the years, sometimes by
//           accident, sometimes on purpose (injected humour and the like).
//         </p>
//         <ul>
//           <li>item item</li>
//           <li>item item</li>
//           <li>item item</li>
//           <li>item item</li>
//         </ul>
//         <ol>
//           <li>item item</li>
//           <li>item item</li>
//           <li>item item</li>
//           <li>item item</li>
//         </ol>
//         <h2>Заголовок второго уровня</h2>
//         <ul>
//           <li>item item</li>
//           <li>item item</li>
//           <li>item item</li>
//           <li>item item</li>
//         </ul>
//       </>
//     ),
//   },
//   {
//     id: 'polyinvest5',
//     title: 'Полиинвест5',
//     tags: ['лендинг', 'ux/ui дизайн', 'wordpress', 'бот'],
//     description:
//       'Разработка лендинга для профессионального портфельного управляющего с целью рассказа о частном автоследовании, его преимуществах и привлечении новых клиентов.',
//     preview: '/assets/images/cases/polyinvest/preview.jpg',
//     link: {
//       url: 'https://antonpolyakov.ru/',
//       text: 'antonpolyakov.ru',
//     },
//     content: (
//       <>
//         <h2>Заголовок второго уровня</h2>
//         <p>
//           t is a long established fact that a reader will be distracted by the readable content of a page when looking
//           at its layout. The point of using Lorem Ipsum is that it has a more-or-less normal distribution of letters, as
//           opposed to using Content here, content here, making it look like readable English. Many desktop publishing
//           packages and web page editors now use Lorem Ipsum as their default model text, and a search for lorem ipsum
//           will uncover many web sites still in their infancy. Various versions have evolved over the years, sometimes by
//           accident, sometimes on purpose (injected humour and the like).
//         </p>
//         <Image src={'/assets/images/placeholder.png'} width={300} height={150} alt={''} />
//         <h2>Заголовок второго уровня</h2>
//         <Image src={'/assets/images/placeholder.png'} width={300} height={150} alt={''} />
//         <h2>Заголовок второго уровня</h2>
//         <p>
//           t is a long established fact that a reader will be distracted by the readable content of a page when looking
//           at its layout. The point of using Lorem Ipsum is that it has a more-or-less normal distribution of letters, as
//           opposed to using Content here, content here, making it look like readable English. Many desktop publishing
//           packages and web page editors now use Lorem Ipsum as their default model text, and a search for lorem ipsum
//           will uncover many web sites still in their infancy. Various versions have evolved over the years, sometimes by
//           accident, sometimes on purpose (injected humour and the like).
//         </p>
//         <Image src={'/assets/images/placeholder.png'} width={300} height={150} alt={''} />
//         <p>
//           t is a long established fact that a reader will be distracted by the readable content of a page when looking
//           at its layout. The point of using Lorem Ipsum is that it has a more-or-less normal distribution of letters, as
//           opposed to using Content here, content here, making it look like readable English. Many desktop publishing
//           packages and web page editors now use Lorem Ipsum as their default model text, and a search for lorem ipsum
//           will uncover many web sites still in their infancy. Various versions have evolved over the years, sometimes by
//           accident, sometimes on purpose (injected humour and the like).
//         </p>
//         <p>
//           t is a long established fact that a reader will be distracted by the readable content of a page when looking
//           at its layout. The point of using Lorem Ipsum is that it has a more-or-less normal distribution of letters, as
//           opposed to using Content here, content here, making it look like readable English. Many desktop publishing
//           packages and web page editors now use Lorem Ipsum as their default model text, and a search for lorem ipsum
//           will uncover many web sites still in their infancy. Various versions have evolved over the years, sometimes by
//           accident, sometimes on purpose (injected humour and the like).
//         </p>
//         <h2>Заголовок второго уровня</h2>
//         <p>
//           t is a long established fact that a reader will be distracted by the readable content of a page when looking
//           at its layout. The point of using Lorem Ipsum is that it has a more-or-less normal distribution of letters, as
//           opposed to using Content here, content here, making it look like readable English. Many desktop publishing
//           packages and web page editors now use Lorem Ipsum as their default model text, and a search for lorem ipsum
//           will uncover many web sites still in their infancy. Various versions have evolved over the years, sometimes by
//           accident, sometimes on purpose (injected humour and the like).
//         </p>
//         <ul>
//           <li>item item</li>
//           <li>item item</li>
//           <li>item item</li>
//           <li>item item</li>
//         </ul>
//         <ol>
//           <li>item item</li>
//           <li>item item</li>
//           <li>item item</li>
//           <li>item item</li>
//         </ol>
//         <h2>Заголовок второго уровня</h2>
//         <ul>
//           <li>item item</li>
//           <li>item item</li>
//           <li>item item</li>
//           <li>item item</li>
//         </ul>
//       </>
//     ),
//   },
// ];

const casesData: CasesPost[] = [];

export default casesData;
